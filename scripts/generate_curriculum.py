...
Usage:
    ...
    python scripts/generate_curriculum.py nav       # Refresh MkDocs navigation
...
NAV_START = "# AUTOGENERATED_NAV_START"
NAV_END = "# AUTOGENERATED_NAV_END"
...
def build_mkdocs_nav(curriculum: Curriculum) -> tuple[list, list[str]]:
    """Build MkDocs nav structure and return missing theory entries."""
    nav = [
        {"Home": "index.md"},
    ...
    missing_theory = []
    curriculum_nav = nav[-1]["Curriculum"]
    curriculum_nav.append("curriculum/index.md")

    for phase in curriculum.phases:
        phase_items: list = [f"curriculum/phase-{phase.id}/index.md"]

        for module in phase.modules:
            theory = module.theory_file
            if not theory:
                missing_theory.append(f"{module.global_id} {module.name}")
                continue

            try:
                rel_path = theory.relative_to(PROJECT_ROOT / "docs")
            except ValueError:
                rel_path = theory

            label = f"{module.global_id} {module.name}"
            phase_items.append({label: str(rel_path).replace("\\", "/")})

        curriculum_nav.append({f"Phase {phase.id}: {phase.name}": phase_items})

    return nav, missing_theory


def render_nav_yaml(nav: list) -> str:
    """Render nav YAML."""
    return yaml.safe_dump({"nav": nav}, sort_keys=False, width=100).rstrip()


def update_mkdocs_nav(nav_yaml: str):
    """Replace nav block in mkdocs.yml between markers."""
    if not MKDOCS_FILE.exists():
        print(f"mkdocs.yml not found at {MKDOCS_FILE}")
        sys.exit(1)

    content = MKDOCS_FILE.read_text()

    if NAV_START not in content or NAV_END not in content:
        print(f"mkdocs.yml missing {NAV_START}/{NAV_END} markers")
        sys.exit(1)

    start_idx = content.index(NAV_START)
    end_idx = content.index(NAV_END)
    if end_idx < start_idx:
        print("mkdocs.yml nav markers are inverted")
        sys.exit(1)

    new_block = f"{NAV_START}\n{nav_yaml}\n{NAV_END}"
    updated = content[:start_idx] + new_block + content[end_idx + len(NAV_END):]

    MKDOCS_FILE.write_text(updated)
...
def cmd_nav():
    """Generate MkDocs navigation from curriculum.yaml."""
    print("📚 Loading curriculum.yaml...")
    curriculum = load_curriculum()

    nav, missing = build_mkdocs_nav(curriculum)
    nav_yaml = render_nav_yaml(nav)

    print("🧭 Updating MkDocs navigation...")
    update_mkdocs_nav(nav_yaml)

    if missing:
        print(f"⚠️ Missing theory files for {len(missing)} modules (nav entries skipped):")
